...
interface ConfirmWithdrawLiquidityModalProps {
  open: boolean
  onClose(): void
  farm: Farm
...
const ConfirmWithdrawLiquidityModal: React.FC<ConfirmWithdrawLiquidityModalProps> = ({
  open,
  onClose,
  farm,
  availableAsset,
  ...
  const estimateGasFee = React.useCallback(async () => {
    try {
      const tx = await withdrawLiquidity(farm, amount, '', undefined, true)
      setFee(
        keyBy(
...
export default ConfirmWithdrawLiquidityModal
